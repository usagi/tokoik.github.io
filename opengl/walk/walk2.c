...
void display(void)
{
  ...
  glLoadIdentity();
  glFrustum(-wx + PARALLAX, wx + PARALLAX, -wy, wy, DISTANCE, RANGE);
  glMatrixMode(GL_MODELVIEW);
  glLoadIdentity();
  glTranslated(-PARALLAX, 0.0, 0.0);
  ...
  glMatrixMode(GL_PROJECTION);
  glLoadIdentity();
  glFrustum(-wx, wx, -wy, wy, DISTANCE, RANGE);
  glMatrixMode(GL_MODELVIEW);
  glLoadIdentity();
  glRotated(r, 0.0, 1.0, 0.0);
  glTranslated(ex, 0.0, ez);
...
#ifdef MULTI
  panangle = atan(wx / DISTANCE) * 180.0 * 2.0 / 3.1415926536;
  scrWidth = w;
  scrHeight = h;
#endif
}
